...
$(document).ready(function () {
	...
	$("#EXPV").popover({
        ...
        placement: 'bottom',
		trigger: 'hover',
		title: '<strong>Sensor EXPV<strong>',
		content: function () {
			return $('#popover-EXPV').html();
		}
    });
	...
	$("#EFB").popover({
        ...
        placement: 'bottom',
		trigger: 'hover',
		title: '<strong>Sensor EFB<strong>',
		content: function () {
			return $('#popover-EFB').html();
		}
    });
	...
	$("#EXV").popover({
        ...
        placement: 'bottom',
		trigger: 'hover',
		title: '<strong>Sensor EXV<strong>',
		content: function () {
			return $('#popover-EXV').html();
		}
    });
	...
	$("#EOM").popover({
        ...
        placement: 'bottom',
		trigger: 'hover',
		title: '<strong>Sensor EOM<strong>',
		content: function () {
			return $('#popover-EOM').html();
		}
    });
	...
	$("#EZM").popover({
        ...
        placement: 'bottom',
		trigger: 'hover',
		title: '<strong>Sensor EZM<strong>',
		content: function () {
			return $('#popover-EZM').html();
		}
    });
...
var app = angular.module('elevatorApp', ['ngSanitize', 'ws.blink']);

app.factory('socket1', function($rootScope){

	var socket = io.connect('http://www.sotefinservice.com:3002/elevator');
	return {
...
app.controller('elevatorCtrl', function($sanitize, $scope, $sce, socket1, socket2) {
	...
	socket1.on('refresh:elevators', function (data) {
		//console.log('Received JSON data :\n' + data.elevators);
		$scope.system = JSON.parse(data.elevators);
		$scope.el1 = $scope.system.elevators[0];
		$scope.el2 = $scope.system.elevators[1];
		$scope.el3 = $scope.system.elevators[2];
	});
		
	socket2.on('refresh:plcio-el', function (data) {
		//console.log('Received JSON data :\n' + JSON.stringify(data.plcio));
		$scope.el1_card = data.plcio[0];	//JSON.parse(data.plcio);
		$scope.el2_card = data.plcio[1];
		$scope.el3_card = data.plcio[2];
	...
	$scope.elevatorClass = function (status) {
		if (status) {
	...
	$scope.elevatorStatus = function (status) {
		if (status) {
			return 'System Ready';
		} else {
