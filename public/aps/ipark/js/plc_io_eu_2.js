...
app.factory('socket', function($rootScope){
	...
	var socket = io.connect('http://www.sotefinservice.com:3005/plc_io_eu');
	return {
...
app.controller('ioCtrl', function($scope, socket) {
	...
	socket.on('paint:plcio-io-eu-1', function (data) {
		var io = JSON.parse(data.plcio);
		$scope.rack_11 = io;
	});
	
	socket.on('paint:plcio-io-eu-2', function (data) {
		var io = JSON.parse(data.plcio);
		$scope.rack_12 = io;
	});
	
	socket.on('paint:plcio-io-eu-3', function (data) {
		var io = JSON.parse(data.plcio);
		$scope.rack_13 = io;
	});
	
	socket.on('paint:plcio-io-eu-4', function (data) {
		var io = JSON.parse(data.plcio);
		$scope.rack_14 = io;
	});
	
	socket.on('refresh:plcio_eu_02', function (data) {
