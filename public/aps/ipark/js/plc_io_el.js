...
app.controller('ioCtrl', function($scope, socket) {
	...
	socket.on('paint:plcio-io01', function (data) {
		//console.log('Received JSON data :\n' + data.plcio);
		var io = JSON.parse(data.plcio);
		$scope.rack_el1 = io;
	});
	
	socket.on('paint:plcio-io02', function (data) {
		//console.log('Received JSON data :\n' + data.plcio);
		var io = JSON.parse(data.plcio);
		$scope.rack_el2 = io;
	});
	
	socket.on('paint:plcio-io03', function (data) {
		//console.log('Received JSON data :\n' + data.plcio);
		var io = JSON.parse(data.plcio);
		$scope.rack_el3 = io;
	});
	
	socket.on('paint:plcio-io04', function (data) {
		//console.log('Received JSON data :\n' + data.plcio);
		var io = JSON.parse(data.plcio);
		$scope.rack_el4 = io;
	});
	
	socket.on('refresh:plcio_el', function (data) {
