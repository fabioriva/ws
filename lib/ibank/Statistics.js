...
exports.operationDaily = function (db_conn, date, done) {
	...
	var d = moment(date, 'YYYY-MM-DD').isValid() ? moment(date).format('YYYY-MM-DD') : moment().tz('Pacific/Auckland').format('YYYY-MM-DD');
...
exports.operationWeekly = function (db_conn, date, done) {
	...
	var d2 = moment(date, 'YYYY-MM-DD').isValid() ? moment(date).format('YYYY-MM-DD') : moment().tz('Pacific/Auckland').format('YYYY-MM-DD');
	var d1 = moment(d2).subtract(1, 'weeks').tz('Pacific/Auckland').format('YYYY-MM-DD');
	...
	for (var i=0; i<7; i++) {
		...
		//console.log(i, d3);
		d3 = moment(d3).add(1, 'days').tz('Pacific/Auckland').format('YYYY-MM-DD');
	}
...
exports.operationMonthly = function (db_conn, date, done) {
	...
	db.query(sql, function(err, query) {
		...
		for (var i in query) {
			//console.log(JSON.stringify(query[i]));
			//query[i].date = moment(query[i].t_history_date).tz('Pacific/Auckland').format('YYYY-MM-DD');
			query[i].date = moment(query[i].date).tz('Pacific/Auckland').format('DD');
			//console.log(JSON.stringify(query[i]));
		}
		// close db connection gracefully
		db.end();
