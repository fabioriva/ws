...
function Alarm (id, system, device, from, to) {
...
Alarm.prototype.getAlarm = function (from, to, done) {
...
Alarm.prototype.setAlarm = function (a, s) {

	var alarms = this.alarms;
	alarms.alarm[a-1].status = s;
...
Alarm.prototype.setAlarms = function (byteIni, byteLen, data, alarms) {
	...
	for (var y=0; y<byteLen; y++) {
		...
		for (var b=0; b<8; b++) {
			...
			{
				alarms.alarm[i].date = moment().tz('Asia/Kolkata').format('YYYY-MM-DD');
				alarms.alarm[i].time = moment().tz('Asia/Kolkata').format('HH:mm:ss');
				...
				// sendMail(
					...
					// 'notify@sotefinservice.com, kamlanagarparkingplcalarm@gmail.com',
